...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==ceil(self.assets.count*1.0/columns)) {
        ...
        if (_numberOfVideos == 0)
            ...
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld", nil), (long)_numberOfVideos];
        else
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld, %ld", nil), (long)_numberOfPhotos, (long)_numberOfVideos];
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    if (photosSelected && videoSelected)
        format = NSLocalizedString(@" %ld ", nil);
    
    else if (photosSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@"%ld ", nil) : NSLocalizedString(@"%ld  ", nil);
...
- (NSString *)accessibilityLabel
{
    ...
    return [label stringByAppendingFormat:NSLocalizedString(@"%ld ", nil), (long)[self.assetsGroup numberOfAssets]];
}
